    ...
    @if($data['library'] !== null)
        @foreach($data['library'] as $library)
            <a href="{{ route('library.item', ['slug' => $library->slug]) }}" class="flex flex-col py-2 px-1.5 hover:bg-gray-100 rounded-md transition ease-in-out duration-500 block cursor-pointer">
                <div x-data class="flex items-center justify-between text-primary-600 hover:dark:text-primary-300">
                    <h3>{{ $library->title ?? '' }}</h3>
                ...
                <cite class="text-sm text-primary-600 hover:dark:text-primary-300">
                    {{ $library->description }}
                </cite>
            </a>
        @endforeach
